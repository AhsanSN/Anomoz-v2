...
<script>
//document.cookie = "timelimit=3; path=/;";
...
now.setTime(time);
document.cookie = 
'currentToken_notf='+currentToken+ '; path=/';
...
now.setTime(time);
document.cookie = 
'currentToken_notf='+refreshedToken+ '; path=/';
    //token received
  }).catch(function(err) {
...
messaging.onMessage(function(payload) {
    var user = getCookie("session_usernumber");
    console.log('Message received. ', payload);
    if (user.slice(0, 6) != (payload.data.sender).slice(0, 6)){
  console.log('Message received 2. ');
  document.getElementById("info_title").innerHTML = payload.notification.title;
  document.getElementById("info_body").innerHTML = payload.notification.body;
  document.getElementById("on_screen_notifications").style.display = "block";
    }
  // ...
});
...
var cookie_notf = getCookie("cookie_notf_i");
function show_box(){
    if (cookie_notf !== '1')//ask condition
    {
        if(Notification.permission != "granted"){
        document.getElementById("notf_bar").style.display = "block";
        }
    }
    
}
setTimeout(show_box, 13000);
...
now.setTime(time);
document.cookie = 
'cookie_notf_i=1; expires=' + now.toUTCString() + 
'; path=/';
}
function no(){
